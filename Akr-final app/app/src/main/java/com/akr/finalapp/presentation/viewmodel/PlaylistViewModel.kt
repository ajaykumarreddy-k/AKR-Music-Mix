...
import com.akr.finalapp.data.model.SmartPlaylistRule
import com.akr.finalapp.data.model.Song
import com.akr.finalapp.data.model.SortOption
import com.akr.finalapp.data.playlist.M3uManager
...
import com.akr.finalapp.data.preferences.TelegramTopicDisplayMode
import com.akr.finalapp.data.ai.AiPlaylistGenerator
...
class PlaylistViewModel @Inject constructor(
    ...
    private fun findFolder(
        targetPath: String,
        folders: List<com.akr.finalapp.data.model.MusicFolder>
    ): com.akr.finalapp.data.model.MusicFolder? {
        val queue: ArrayDeque<com.akr.finalapp.data.model.MusicFolder> = ArrayDeque(folders)
    ...
    private fun com.akr.finalapp.data.model.MusicFolder.collectAllSongs(): List<Song> {
    ...
    private fun sortPlaylistsList(
        playlists: List<com.akr.finalapp.data.model.Playlist>,
        sortOption: SortOption
    ): List<com.akr.finalapp.data.model.Playlist> {
        return when (sortOption) {
            SortOption.PlaylistNameAZ -> playlists.sortedWith(
                compareBy<com.akr.finalapp.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistNameZA -> playlists.sortedWith(
                compareByDescending<com.akr.finalapp.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreated -> playlists.sortedWith(
                compareByDescending<com.akr.finalapp.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreatedAsc -> playlists.sortedWith(
                compareBy<com.akr.finalapp.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
